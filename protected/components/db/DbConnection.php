...
class DbConnection extends Component
{
	...
	public $driverMap=array(
		'pgsql'=>'PgsqlSchema',    // PostgreSQL
		'mysqli'=>'MysqlSchema',   // MySQL
		'mysql'=>'MysqlSchema',    // MySQL
		'sqlite'=>'SqliteSchema',  // sqlite 3
		'sqlite2'=>'SqliteSchema', // sqlite 2
		'mssql'=>'MssqlSchema',    // Mssql driver on windows hosts
		'dblib'=>'MssqlSchema',    // dblib drivers on linux (and maybe others os) hosts
		'sqlsrv'=>'MssqlSchema',   // Mssql
		'oci'=>'OciSchema',        // Oracle driver
	);
	...
	protected function open()
	{
		if($this->_pdo===null)
		{
			if(empty($this->connectionString))
				throw new Exception('DbConnection.connectionString cannot be empty.');
			try
			{
			...
			{
				throw new Exception('DbConnection failed to open the DB connection. (code=' . (int)$e->getCode() . ')');
			}
		}
	}
	...
	public function getSchema()
	{
		...
		else
		{
			...
			if(isset($this->driverMap[$driver]))
				return $this->_schema = WireFrame::createComponent($this->driverMap[$driver], $this);
			else
				throw new Exception('DbConnection does not support reading schema for ' .$driver. ' database.');
		}
	} */
